...
- 军衔、头衔按下方"军衔、头衔"表翻译；Captain 根据语境分译：小队长用"队长"，舰长用"舰长"。
- `Delta` 音译为 `德尔塔`，不保留英文。
...
| 原文 | 译文 | 说明 |
| --- | --- | --- |
| Captain Martz | 马茨舰长 | 角色名；人名音译，头衔"舰长" |
| Martz | 马茨 | `Captain Martz` 的简称 |
| Captain Talbot | 泰伯特舰长 | 角色名；人名音译，头衔"舰长" |
...
- 无识别码的装备、武器、载具按语义翻译。
- 拾取信息（PickupMessage）中出现的裸武器名，多数指该武器的弹药（如 `DC-17m Blaster` → 弹匣、`DC-17m Sniper` → 狙击弹），只有极个别是拾取武器本体；按关卡实际语境判断。
- `grenade` 泛指时译为「手雷」；投掷型 `detonator` 译为「榴弹」，安置型引爆装置语境译为「起爆器」（如 `remote mine detonator` → 遥控地雷起爆器）；布设投掷雷陷阱的动作用「布雷」等说法。
...
| DC-17m Blaster / DC-17m Blaster Clips（拾取信息） | DC-17m爆能枪弹匣 | 弹药拾取；拾取信息中的裸武器名多为弹药 |
| DC17m Blaster Rifle | DC-17m爆能步枪 | 型号保留，类别翻译 |
| DC-17m Blaster Attachment | DC-17m爆能枪附件 | 型号保留，类别翻译 |
| DC-17m Sniper Attachment / DC17m Sniper Attachment / DC17mSniper | DC-17m狙击附件 | 武器附件本体 |
| DC-17m Sniper（拾取信息） | DC-17m狙击弹 | 弹药拾取；与狙击附件是不同概念，拾取信息中的裸武器名多为弹药 |
| demolition charge / demolition charges / demo charge | 炸药 | 普通装备名 |
| EC Detonator / EC Detonators | 静电荷榴弹 | 复数不在中文中体现；与 EMP 区分 |
| ECD | 静电荷榴弹 | 对话中对 EC 榴弹的简称，译出全称 |
| EMP Grenade / EMP Grenades | 电磁脉冲榴弹 | 复数不在中文中体现 |
| EWHB-12 | EWHB-12 | 仅型号 |
| EWHB-12 Mounted Gun | EWHB-12安装炮塔 | 型号保留，类别翻译 |
| EWeb Turret / EWebTurret | EWeb炮塔 | 型号保留，类别翻译 |
| Flash Detonator / Flash Detonators / Flashbang Detonator / Flashbang Detonators / Flashbang / Flashbangs / FlashBang | 闪光手雷 | 同一装备；别名不另译为不同装备 |
| Flight Recorder / flight recorder | 飞行记录仪 | 普通装备名 |
| Force Pike / ForcePike | 力矛 | 武器名；不译"原力" |
| ForcePike Cell | 力矛电池 | 弹药 |
...
| Sonic Detonator / Sonic Detonators / SonicDetonator | 声波榴弹 | 复数不在中文中体现 |
| StickyGrenade / Sticky Grenade | 粘性手雷 | 多人模式物品 |
| Thermal Detonator / Thermal Detonators | 热能榴弹 | 复数不在中文中体现 |
...
| Wookiee Homing Rocket Launcher / Wookiee Rocket Launcher / Wookie Rocket Launcher / Rocket Launcher / RocketLauncher | 伍基人制导导弹发射器 | 种族名使用译名；本作中 Rocket Launcher 统一指此武器 |
| DC-17m Anti-Armor Charge Packs / DC-17m AntiArmor ammo | DC-17m反装甲充能包 | 弹药拾取，统一用"充能包" |
...
| scurrier / scurriers | 沙跳鼠 | 星战世界观中的鼠形小兽 |
...
| Grenade Position / grenade position | 手雷阵地 | 小队投掷/火力位置 |
